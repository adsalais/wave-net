...
//!
//! Copied from `wave_bitnet::config`; the only difference is the `adapt_decay` bound — `wave_driven`
//! decays adaptation with a geometric multiply (ρ = 1 − 2^−adapt_decay) that has no fixed-point dead
//! zone, so the `adapt_decay <= ADAPT_SHIFT` cap is replaced by the wider `1..=MAX_ADAPT_DECAY`.
...
pub struct LayerConfig {
    ...
    pub adapt_decay: u8,       // sets the geometric decay ratio ρ = 1 − 2^−adapt_decay (>= 1)
}
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (z, lc) in self.layers.iter().enumerate() {
            ...
            for t in &lc.topology {
                let n = crate::wave_driven::synapse::neigh_size(t.radius);
