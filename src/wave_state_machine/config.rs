...
impl Config {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (z, lc) in self.layers.iter().enumerate() {
            ...
            if lc.adapt_decay == 0 || lc.adapt_decay as u32 > crate::wave_state_machine::neurons::ADAPT_SHIFT {
                return Err(format!(
                    "layer {z}: adapt_decay must be in 1..={} (ADAPT_SHIFT; larger reintroduces the fixed-point dead zone)",
                    crate::wave_state_machine::neurons::ADAPT_SHIFT
                ));
            }
        }
        Ok(())
...
mod tests {
    ...
    fn rejects_adapt_decay_above_shift() {
        use crate::wave_state_machine::neurons::ADAPT_SHIFT;
        let mut c = Config::demo();
