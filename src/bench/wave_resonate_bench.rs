...
mod tests {
    ...
    fn make_ff(seed: u64, size: u32, layers: usize, up_count: u32, up_radius: u32, theta_c: f32, b_off: (f32, f32)) -> (Network, Vec<Vec<Edge>>) {
        let lc = LayerConfig {
    ...
    fn ff_cfg() -> TaskCfg {
        TaskCfg { size: 16, present: 6, delay: 4, read: 6, holdout: 200, readout_lr: 0.02, hidden_lr: 2.0, rate_reg: 0.0, rate_target: 0.1 }
    }
    ...
    fn wave_resonate_liveness_probe() {
        ...
        for &b_off in &[(0.0f32, 0.2f32), (0.1, 1.0)] {
            for &theta_c in &[0.05f32, 0.1, 0.2, 0.5, 1.0] {
                let (mut net, _e) = make_ff(seed, 16, 4, 24, 3, theta_c, b_off);
    ...
    fn wave_resonate_ff_diagnose() {
        ...
        for &layers in &[2usize, 3, 4] {
            let (mut n0, e0) = make_ff(seed, 16, layers, 24, 3, 0.1, (0.0, 0.2));
            let mut c0 = ff_cfg();
            c0.hidden_lr = 0.0;
            let (ro, _) = train_and_eval_best(&mut n0, &e0, seed, seed, &c0, single_task, 100, 4, 1500);
            let (mut n1, e1) = make_ff(seed, 16, layers, 24, 3, 0.1, (0.0, 0.2));
    ...
    fn wave_resonate_ff_trains_above_chance() {
        ...
        let seed = 0xE9_0B_0A17u64;
        let (mut net, entries) = make_ff(seed, 16, 4, 24, 3, 0.1, (0.0, 0.2));
        let cfg = ff_cfg();
